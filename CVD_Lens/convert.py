...
def tensor_uint8(im):
    """clamp the tensor to [0, 1]"""
    # return torch.tensor(torch.clamp(im,0.,1.0))
    return torch.clamp(im, 0., 1.0)


def tensor_float32(im):
    """ im : tensor in range [0, 1]
    cast the tensor im to float32 and in the range of [0, 1.0]
    """
    # return torch.tensor(im, dtype=torch.float32) * 0.5 + 0.5
    # return (im.to(torch.float32) * 0.5 + 0.5)
...
def tensor_linearRGB_from_sRGB(im):
...
def bin_mask(im):
    im_bi = torch.zeros(im.shape[1], im.shape[2])
    white_mask = im.mean(dim=0) > 0.5
...
def tensor_sRGB_from_linearRGB(im):
    out = torch.zeros_like(im)
    # Make sure we're in range, otherwise gamma will go crazy.
...
def tensor_apply_color_matrix(im, m, device):
    M = torch.tensor(m.T, dtype=torch.float32).to(device)
    return im @ M
