from CVD_Lens import convert
from CVD_Lens.utils import array_to_C_decl, normalized
...
class Simulator (ABC):
    ...
    def __init__(self):
        ...
        self.imageEncoding = convert.ImageEncoding.SRGB # 0

    def simulate_cvd (self, image_srgb_uint8, deficiency: Deficiency, severity: float, device):
        ...
        """
        #im_linear_rgb = convert.as_float32(image_srgb_uint8)        # 将array从[0, 255] 转换成 [0, 1]
        im_linear_rgb = convert.tensor_float32(image_srgb_uint8)    # 将tensor类型转换为torch.float(32)
        if (self.imageEncoding == convert.ImageEncoding.SRGB):  # Convert sRGB to linearRGB, removing the gamma correction
            #im_linear_rgb = convert.linearRGB_from_sRGB(im_linear_rgb)
            im_linear_rgb = convert.tensor_linearRGB_from_sRGB(im_linear_rgb)
        elif (self.imageEncoding == convert.ImageEncoding.GAMMA_22):
            im_linear_rgb = convert.linearRGB_from_gamma22(im_linear_rgb)

        im_cvd_linear_rgb = self._simulate_cvd_linear_rgb(im_linear_rgb, deficiency, severity, device)

        if (self.imageEncoding == convert.ImageEncoding.SRGB):
            #im_cvd_float = convert.sRGB_from_linearRGB(im_cvd_linear_rgb)
            im_cvd_float = convert.tensor_sRGB_from_linearRGB(im_cvd_linear_rgb)
        elif (self.imageEncoding == convert.ImageEncoding.GAMMA_22):
        ...
        return convert.tensor_uint8(im_cvd_float)   # 将tensor clamp到[0.0, 1.0],便于后期的plt.imshow和SSIM Loss计算

    @abstractmethod
    def _simulate_cvd_linear_rgb (self, image_linear_rgb_float32, deficiency: Deficiency, severity: float, device):
...
class Simulator_Machado2009 (Simulator):
    ...
    def _simulate_cvd_linear_rgb (self, image_linear_rgb_float32, deficiency: Deficiency, severity: float, device):
        ...
        return convert.tensor_apply_color_matrix(image_linear_rgb_float32, m, device)
