...
def setup_training_loop_kwargs(
    ...
    if load:
        args.load_from_latest = True
    else:
        args.load_from_latest = False
    ...
    cfg_specs = {
        'auto': dict(ref_gpus=-1, kimg=15000, mb=-1, mbstd=-1, fmaps=-1, lrate=-1, gamma=-1, ema=-1, ramp=0.05, map=2),
    ...
    args.G_kwargs = dnnlib.EasyDict(class_name='training.CVD_networks.CVD_Generator', z_dim=args.dim,  # w_dim=args.dim,
    ...
    args.Q_kwargs = dnnlib.EasyDict(class_name='training.CVD_networks.CVD_Q', latent_size=args.dim,
                                    block_kwargs=dnnlib.EasyDict(), epilogue_kwargs=dnnlib.EasyDict())
    ...
    args.Q_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=spec.lrate, betas=[0, 0.99], eps=1e-8)
    ...
    # ------------------------------------
    # hyper-parameters: latent_size, loss_mode
    # ------------------------------------
    desc += f'-latent{latent_size}'

    args.loss_mode = loss_mode.split('_')[0]
    if args.loss_mode == 'both':
        args.loss_para = dict(CL_para=float(loss_mode.split('_')[1]), SL_para=float(loss_mode.split('_')[2]))
    elif args.loss_mode == 'all':
        args.loss_para = dict(CL_para=float(loss_mode.split('_')[1]), SL_para=float(loss_mode.split('_')[2]), hL_para=float(loss_mode.split('_')[3]))
    elif args.loss_mode in ['no', '']:
        args.loss_para = None
    else:
        args.loss_para = float(loss_mode.split('_')[1])
    desc += f'-{loss_mode}'

    if single_latent is not None:
        args.single = True
        args.type = single_latent.split('_')[0]
        args.degree = float(single_latent.split('_')[1])
        desc += f'-single_latent_{single_latent}'
    else:
        args.single = False
    assert isinstance(args.single, bool)
...
@click.option('--load', help='load from the latest checkpoint [default: false]', type=bool, metavar='BOOL')
# Dataset.
...
# debug hyper-parameters
@click.option('--latent_size', help='the dimensions of latent code', required=True, type=int, metavar='INT')
@click.option('--loss_mode', help='the mode of loss function "contentLoss_para", "ssimLoss_para",  "both_para"',
              required=True, type=click.STRING)
@click.option('--single_latent', help='use single latent code', type=click.STRING)
@click.option('--dis_dim', help='the different dim', type=click.STRING)
def main(ctx, outdir, dry_run, **config_kwargs):
    ...
    for idx in range(len(prev_run_dirs)):
        prev_id.append(prev_run_dirs[idx].split('-')[0])
        prev_name.append(prev_run_dirs[idx][6:])
    prev_dic = dict(zip(prev_name, prev_id))

    if args.load_from_latest and run_desc in prev_name:
        args.run_dir = os.path.join(outdir, prev_dic[run_desc] + '-' + run_desc)
    else:
    ...
    print(f'dimension of latent:    {args.dim}', )
