...
from . import feature_similarity_index
from . import structural_similarity
from . import contrast_preservation_rate
from . import color_information_loss
from . import VGG_perceptual_loss
...
def FSIMc(opts):
    ...
    fsimc = feature_similarity_index.compute_fsimc(opts, num_gen=100)
    return dict(fsimc=fsimc)

@register_metric
def SSIM(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    ssim = structural_similarity.compute_ssim(opts, num_gen=100)
    return dict(ssim=ssim)

@register_metric
def CPR(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    cpr = contrast_preservation_rate.compute_cpr(opts, num_gen=100)
    return dict(cpr=cpr)

@register_metric
def ColorInfoLoss(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    CL = color_information_loss.compute_CL(opts, num_gen=100)
    return dict(colorInfoLoss=CL)

@register_metric
def VGG(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    vgg_loss = VGG_perceptual_loss.compute_vgg(opts, num_gen=100)
    return dict(perceptualLoss=vgg_loss)
