...
# ----------------------------------------------------------------------------
def open_colored_mnist(tarball: str, *, max_images: Optional[int]):
    images = []
    labels = []
    ...
    with tarfile.open(tarball, 'r:gz') as tar:
        for batch in range(1, 7):
            # print('batch:', batch)
            # mnist_10color_jitter_var_0.020.npy
            member = tar.getmember(f'colored_mnist/mnist_10color_jitter_var_0.0{(batch + 3) * 5}.npy')
            with tar.extractfile(member) as file:
                data = np.load(file, encoding='latin1', allow_pickle=True).item()
                # print('keys_test', data.keys())
                labels.append(data['train_label'])
                data = data['train_image'].reshape(-1, 28, 28, 3)
    ...
    #print('images_list_shape : ', len(images))
...
def open_dataset(source, *, max_images: Optional[int]):
    if os.path.isdir(source):
        ...
        elif os.path.basename(source) == 'colored_mnist.tar.gz':
            return open_colored_mnist(source, max_images=max_images)
